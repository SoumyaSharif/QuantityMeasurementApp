...
public class AuthController {
	...
	@GetMapping("/success")
	public ResponseEntity<Map<String, Object>> authSuccess(@RequestParam String token) {
		...
		Map<String, Object> authResponse = Map.of("token", token, "email", user.getEmail(), "name", user.getName(), "pictureUrl", user.getPictureUrl(), "role", user.getRole().name(), "expirationMs", appProperties.getJwt().getExpirationMs());
	...
	public ResponseEntity<Map<String, String>> loginWithGoogle() {
		return ResponseEntity.ok(Map.of("message", "Open this URL in your browser to login with Google", "loginUrl",
				"http://localhost:8081/oauth2/authorization/google", "note",
